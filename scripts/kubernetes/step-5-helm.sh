...
log_step "Step 5. Helm 컴포넌트 설치 (${CP_01_IP})"
...
scp_file "${tmp_helm}" "${CP_01_IP}" "~/05.post-install-helm.sh"
rm -f "${tmp_helm}"

# 원격 실행
remote_exec "${CP_01_IP}" "Helm 컴포넌트 설치" "bash ~/05.post-install-helm.sh"
